...
GP2040-CE 支持多种控制台，以下列出关于控制台兼容性的常见问题。
...
许多用户错误地将控制器插入转换器/适配器，然后再将转换器/适配器插入控制台。这种连接方式并非通过认证的预期用法，会导致转换器/适配器处理输入从而增加延迟。
...
PS4 和 PS5 使用认证机制确保仅限授权控制器在控制台上使用。如果设备未实现该认证机制，控制器将在 8 分钟后超时。
...
PS4 输入模式是一种允许 GP2040-CE 与 PS4 控制台兼容的输入模式。可以通过上传密钥文件或使用带通过认证设备的 USB 主机端口来绕过 8 分钟超时。
...
PS5 输入模式是一种通过另一控制器或加密狗进行认证，从而令 GP2040-CE 与 PS4 或 PS5 控制台兼容的输入模式。
...
以下设备适用于 PS5 并且支持通过认证。这些设备搭载 `NXP7105` 芯片，可与 PS5 控制台认证：
...
- **PS4 输入模式**：设备成功通过 PS4 控制台认证后，显示的输入模式将从 `PS4` 变为 `PS4:AS`。
- **PS5 输入模式**：设备成功通过 PS5 控制台认证后，显示的输入模式将从 `PS5` 变为 `PS5:AS`。
...
PlayStation 控制台每 30 秒左右重复发送认证请求。如果请求失败（即未插入通过认证设备），约 8 分钟后控制器将停止响应。一旦发生这种情况，只能通过重置控制器或重新插拔控制器恢复。
