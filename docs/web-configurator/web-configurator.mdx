...
GP2040-CE包含一个内置的基于Web的配置应用程序，您可以通过在将控制器插入PC时按住<Hotkey buttons={["S2"]}/>，或在控制器插入时按住<Hotkey buttons={["S2", "B3", "B4"]}/>五秒钟来启动它。然后，在Web浏览器中访问[http://192.168.7.1](http://192.168.7.1)即可开始配置。
...
并非所有的Android和iOS设备都支持Web配置。在我们的测试中，一些Android设备可以工作，而另一些则不行。大多数基于USB-C的iOS设备都能工作。我们没有记录哪些设备可以或不可以工作。

:::

## 在Windows PC上访问Web配置时遇到问题

如果您的Windows PC无法访问GP2040-CE控制器的内置Web配置，请尝试以下步骤：

1 - **确保您处于网页配置模式**

  - 确保您的控制器处于网页配置模式。
  - 验证USB线缆连接牢固，并支持数据传输（有些线缆只支持供电）。

2 - **尝试不同的浏览器**

  - 某些浏览器可能会阻止本地设备访问。请尝试Chrome、Firefox或Edge。
  - 暂时禁用浏览器扩展（如广告拦截器）。

3 - **检查URL**

  - Web配置的URL是[http://192.168.7.1](http://192.168.7.1)。
...
除了上述情况，您的PC上安装的驱动程序可能存在问题。由于存在导致系统问题的风险，GP2040-CE团队不会协助解决基于驱动程序的问题，也不会建议安装或卸载驱动程序。

:::

:::note

Linux发行版可能需要一些额外的步骤才能访问Web配置器；请参阅[Linux安装](https://www.google.com/search?q=%23linux-setup)。
...
Web配置器会自动运行，您只需要能够访问它即可。某些配置会自动设置路由，因此现在请在浏览器中尝试[http://192.168.7.1](http://192.168.7.1)。如果无法加载，请尝试通过以下命令手动为接口配置IP：`sudo ifconfig enp6s0f1u3 192.168.7.2`。

无论您是否需要手动添加IP，您都应该会得到类似如下的路由：
