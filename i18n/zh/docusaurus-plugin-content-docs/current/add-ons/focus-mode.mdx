...
- `锁定宏` - 启用后，在专注模式下宏触发将无法激活宏序列。
- `锁定按键` - 启用后，列出的输入在专注模式下将不会被发送。随着输入的添加，将出现新的下拉菜单以供选择更多输入。

请注意，专注模式引脚可以在 [GPIO 引脚映射](../web-configurator/menu-pages/gpio-pin-mapping) 页面上设置。
