...
```mermaid
graph LR
转换器/适配器/加密狗 -- 游戏控制器上的 <br> USB 接口 --> 游戏控制器([游戏控制器]) -- 主机上的 <br> USB接口 --> 主机{{主机}}
```
...
```mermaid
graph LR
游戏控制器([游戏控制器]) -- 转换器上的 <br> USB接口 --> 转换器/适配器/加密狗 -- 主机上的 <br> USB接口 --> 主机{{主机}}
```
...
- 对于 PS4，可使用 [PS4 输入模式](../web-configurator/menu-pages/01-settings.mdx#额外的-ps4-设置) 或 [PS5 输入模式](../web-configurator/menu-pages/01-settings.mdx#额外的-ps5-设置)。
- 对于 PS5，只能使用 [PS5 输入模式](../web-configurator/menu-pages/01-settings.mdx#额外的-ps5-设置)。

### 如何让我的 GP2040-CE 设备在 PS4 或 PS5 上工作？

- **PS4**：需要使用 [PS4 输入模式](../web-configurator/menu-pages/01-settings.mdx#额外的-ps4-设置) 或 [PS5 输入模式](../web-configurator/menu-pages/01-settings.mdx#额外的-ps5-设置)。
- **PS5**：不能使用 [PS4 输入模式](../web-configurator/menu-pages/01-settings.mdx#额外的-ps4-设置)，必须使用 [PS5 输入模式](../web-configurator/menu-pages/01-settings.mdx#额外的-ps5-设置)。
...
更多信息请参阅文档中 `Web Configurator > Settings` 部分的 [PS4 输入模式](../web-configurator/menu-pages/01-settings.mdx#额外的-ps4-设置)。
...
为使用此设置，需要在 [网页配置器 > 外围设备映射 > USB 主机](../web-configurator/menu-pages/03-peripheral-mapping.mdx#usb-主机) 中配置 USB 外围设备。
...
更多信息，请参考文档中 `网页配置器 - 设置` 部分的 [PS5 输入模式](../web-configurator/menu-pages/01-settings.mdx#额外的-ps5-设置)。

#### 在 PS5 和 PS4 输入模式下，我应使用什么认证设备？

:::note

PS4 和 PS5 共享设备列表，因为游戏控制器认证为已授权的 PS4 游戏控制器。如果适用于 PS5，那么也适用于 PS4。
...
通常的选择是将游戏控制器插入 PS4 或 PS5，等待 8 分钟，验证游戏控制器是否仍然响应。

:::info

对于上述的 MagicBoots 和 N5，启动时加密狗上的指示灯应闪烁一会，之后保持常亮。如果灯常亮，那么游戏控制器应该可以正常兼容 PS5 并避免超时。

:::

#### 插入游戏控制器后可否移除 PS4/PS5 认证设备？

PlayStation 主机每 30 秒左右重复发送认证请求。如果请求失败（即未插入通过认证设备），约 8 分钟后游戏控制器将停止响应。一旦发生这种情况，只能通过重置游戏控制器或重新插拔游戏控制器恢复。
...
:::caution Xbox 360 游戏控制器  

Xbox 360 无线控制器、Xbox 360 有线控制器和其他授权的 Xbox 360 游戏控制器无法与此插件兼容。  

:::

### 如何判断 Xbox 360 输入模式插件是否正常工作？

由于需要使用多种设备进行传递认证，因此无法在不将游戏控制器连接到主机并尝试使用之前，确认输入模式是否正常工作。

对于上述的 MagicBoots 加密狗，该加密狗上有一个指示灯。启动时，指示灯会闪烁，然后在片刻后变为常亮。如果指示灯亮起且保持常亮，您的游戏控制器**应该**是兼容的，您可以正常使用游戏控制器。

### 插入游戏控制器后，我可以移除 Xbox 360 验证设备吗？

Xbox 360 主机在游戏控制器插入主机时，仅会进行一次认证。一旦完成认证，传递认证设备可以移除。但建议始终将设备保持连接状态。
...
为了实现 Xbox One、Xbox Series S 或 Xbox Series X 的兼容性，您需要使用 [Xbox One 输入模式](../web-configurator/menu-pages/01-settings.mdx#额外的-xbox-one-设置)。
...
Xbox One 输入模式允许 GP2040-CE 通过另一款游戏控制器或加密狗完成认证，从而实现对 Xbox One、Xbox Series S 和 Xbox Series X 的兼容性。

欲了解更多信息，请参阅文档中 “Web Configurator - Add-ons” 部分的 [Xbox One 输入模式](../web-configurator/menu-pages/01-settings.mdx#额外的-xbox-one-设置)。

### 哪种游戏控制器或加密狗可用于 Xbox One 输入模式？
...
:::caution Xbox One 游戏控制器

Xbox 无线控制器、Xbox One 精英手柄和其他授权的 Xbox One 游戏控制器无法与此插件兼容。

:::

### 如何判断 Xbox One 输入模式插件是否正常工作？
...
### 插入游戏控制器后，我可以移除 Xbox One 验证设备吗？

Xbox One、Xbox Series S 和 Xbox Series X 主机在游戏控制器插入主机时，仅会进行一次认证。一旦完成认证，传递认证设备可以移除。但建议始终将设备保持连接状态。
...
通过测试，我们发现如果您计划使用 USB 集线器，建议在 RP2040 高级扩展板上通过网页配置器禁用 RGB LED。可以通过将 GPIO 引脚改为 `-1` 并保存来禁用 RGB LED。  
