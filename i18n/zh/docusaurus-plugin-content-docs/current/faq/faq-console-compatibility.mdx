...
```mermaid
graph LR
转换器/适配器/加密狗 -- 控制器上的 <br> USB 接口 --> 控制器([控制器]) -- 主机上的 <br> USB接口 --> 主机{{主机}}
```
...
```mermaid
graph LR
控制器([控制器]) -- 转换器上的 <br> USB接口 --> 转换器/适配器/加密狗 -- 主机上的 <br> USB接口 --> 主机{{主机}}
```
...
Xbox 无线控制器、Xbox One 精英控制器和其他授权的 Xbox One 控制器无法与此插件兼容。
