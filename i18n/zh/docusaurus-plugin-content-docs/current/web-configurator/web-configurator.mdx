...
GP2040-CE 包含一个内置的基于 Web 的配置应用程序，您可以通过在将游戏控制器插入 PC 时按住 <Hotkey buttons={["S2"]}/>，或在游戏控制器插入时按住 <Hotkey buttons={["S2", "B3", "B4"]}/> 五秒钟来启动它。然后，在 Web 浏览器中访问 [http://192.168.7.1](http://192.168.7.1) 即可开始配置。
...
如果您的 Windows PC 无法访问 GP2040-CE 游戏控制器的内置网页配置器，请尝试以下步骤：
...
5 - **尝试另一个 USB 端口 / PC / 数据线**

  - 在不同的端口或不同的计算机上进行测试，或更换另一根数据线。
...
当您在按住 <Hotkey buttons={["S2"]}/> 的同时插入游戏控制器时，如果您运行 `dmesg`，您应该会在内核日志中看到它已连接：
...
在上面的示例中，**enp6s0f1u3** 是您游戏控制器的虚拟以太网接口。如果您没有看到第一行 `rndis_host`，请确保您的内核中已编译 `CONFIG_USB_NET_RNDIS_HOST` 或将其作为模块。
