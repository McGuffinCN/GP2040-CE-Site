...
![GP2040-CE 配置器 - 显示屏配置](../../assets/images/gpc-display-config.png)

## 硬件选项

- `启用` - 打开/关闭显示模块。

![GP2040-CE 配置器 - 显示屏配置 - 屏幕选项](../../assets/images/gpc-display-config-screen-options.png)
...
![GP2040-CE 配置器 - 显示屏配置 - 布局选项](../../assets/images/gpc-display-config-layout-options.png)
...
![GP2040-CE 配置器 - 显示屏配置 - 模式选项](../../assets/images/gpc-display-config-mode-options.png)
...
![GP2040-CE 配置器 - 显示示例](../../assets/images/gpc-display-example.png)
...
- [输入模式](../../usage.mdx#输入模式) - 显示当前的输入模式：
  ...
  - `HID-KB` - 键盘
  - `PS4` - 游戏控制器模式设置为 `Controller`，认证成功后显示为 `PS4:AS`
  - `PS4:AS` - [PS4 输入模式](../../web-configurator/menu-pages/01-settings.mdx#额外的-ps4-设置) 插件认证成功
  - `PS5` - 游戏控制器模式设置为 `Arcade Stick`，认证成功后显示为 `PS5:AS`
...
- 连发环 - 当在某个按键上启用连发模式时，该按键将显示一个较小的内环作为指示器。禁用时，该按键将消失。
