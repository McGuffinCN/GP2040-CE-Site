...
- `LED 布局` - 选择游戏控制器/按键的布局。这用于静态主题和某些每按键动画。
...
![GP2040-CE 配置器 - RGB Case LED](../../assets/images/gpc-rgb-case-leds.png)

Case RGB LED 允许您在设备内部或外部添加额外的 RGB LED，以增强 RGB 效果。

`颜色类型` 有以下选项：
- `关闭` - 无 Case RGB LED。
- `环境` - Case RGB LED 将独立于按键 LED 设置运行。
- `关联` - Case RGB LED 将匹配按键 LED 设置。
...
| <Hotkey buttons={["S2", "B4"]}/>        | 增加 Case LED 的亮度。                              |
| <Hotkey buttons={["S2", "B2"]}/>        | 减少 Case LED 的亮度。                              |
| <Hotkey buttons={["S2", "R1"]}/>        | 切换 Case LED 的静态颜色到下一个颜色。              |
| <Hotkey buttons={["S2", "R2"]}/>        | 切换 Case LED 的静态颜色到上一个颜色。              |
| <Hotkey buttons={["S2", "L1"]}/>        | 切换到环境模式下的下一个效果。                      |
| <Hotkey buttons={["S2", "L2"]}/>        | 切换到环境模式下的上一个效果。                      |

:::

![GP2040-CE 配置器 - RGB Case LED 设置](../../assets/images/gpc-rgb-case-leds-settings.png)

选择 `环境` 或 `关联` 模式时，您还需要指定 Case LED 的起始索引以及 Case LED 的数量。
- `RGB LED 索引` - Case RGB LED 在链中的起始位置。
- `RGB LED 数量` - 用于 Case RGB LED 的 LED 数量。
