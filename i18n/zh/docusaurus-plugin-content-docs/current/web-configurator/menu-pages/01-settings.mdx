...
![GP2040-CE 配置器 - 输入模式设置](../../assets/images/gpc-settings-input-mode.png)

- `当前输入模式` - 选择此连接设备在通电时启动的主要输入模式（XInput、 DInput、 Switch、 PS4 、 HID-Keyboard 等）。此选择将在断开连接并重新插入游戏控制器后保持不变。
...
![GP2040-CE 配置器 - USB 覆盖](../../assets/images/gpc-settings-usboverride.png)
...
![GP2040-CE 配置器 - 键盘映射](../../assets/images/gpc-settings-keyboard-mapping.png)

### 额外的 PS4 设置

![GP2040-CE 配置器 - 额外的 PS4 设置](../../assets/images/gpc-settings-ps4.png)

- `触摸板(Touchpad)和分享键(Share)互换` - 分享键现在将映射到 A2 而不是 S1 ，触摸板现在将映射到 S1 而不是 A2 。
- `识别模式` 是一种允许在需要真正 DualShock 4 的情况下使用的选项。由于技术原因，这并不会使设备成为真正的 DS4 ，但它确实使固件行为更像 DS4 。
  - `主机模式` - 这是大家熟悉的模式。它在 PS4/PS5 上工作，并具有您期望的授权限制。
  - `串流/模拟模式` - 用于只能使用 DualShock 4 的情况，但不一定需要任何形式的透传授权。 PS Remote Play 仅允许“真实”的 DualShock 4/DualSense 游戏控制器，但目前似乎不需要授权。带有 PADEMU USB 游戏控制器驱动程序的 Open PS2 Loader (OPL) 版本仅寻找“真实”的 DualShock 3/4 游戏控制器，但不需要授权即可工作。
在直接使用 PS4/PS5 主机时，使用主机模式将允许游戏控制器按预期运行。这是默认模式。在直接使用串流/模拟模式时，由于技术原因，游戏控制器不会被检测到。
在通过 PS Remote Play 游戏时，由于主机模式无法显示为真实的 DualShock 4 游戏控制器，因此不会被检测到。使用第三方串流客户端（如 Chiaki）则没有此限制。
除非 PS Remote Play 有具体变化，否则串流/模拟模式不需要授权。
- `认证设置`
  - `上传密钥文件` - 上传从 DualShock 4 获取的密钥文件以认证并绕过 PS4 的 8 分钟超时限制
  - `主机 USB` - 使用加密狗、转换器或授权的 PS4 游戏控制器认证您的 GP2040-CE 设备

<Tabs groupId="authentication-settings" defaultValue="host-usb">

<TabItem value="upload-key-file" label=" 上传密钥文件 " default>

![GP2040-CE 配置器 - 额外的 PS4 设置](../../assets/images/gpc-settings-ps4-upload-key-files.png)
...
![GP2040-CE 配置器 - 额外的 PS4 设置](../../assets/images/gpc-settings-ps4-host-usb.png)

要使用此设置，必须在 [网页配置器 > 外围设备映射 > USB 主机](./03-peripheral-mapping.mdx#usb-主机) 中配置 USB 外围设备。
...
![GP2040-CE 配置器 - 额外的 PS5 设置](../../assets/images/gpc-settings-ps5.png)
...
![GP2040-CE 配置器 - 额外的 PS5 设置](../../assets/images/gpc-settings-ps5-host-usb.png)
...
![GP2040-CE 配置器 - 额外的 Xbox One 设置](../../assets/images/gpc-settings-xbox-one.png)
...
![GP2040-CE 配置器 - 游戏控制器设置](../../assets/images/gpc-settings-gamepad.png)

- `方向模式` - 选择默认的方向模式（十字键、左摇杆或右摇杆）。
- `SOCD 覆盖模式` - 选择默认的 SOCD 覆盖模式（回中、后输入优先、先输入优先、关闭）。请注意， PS4 、 PS3 和 Nintendo Switch 模式不支持关闭 SOCD 设置，将默认使用回中 SOCD。
- `强制设置模式` - 允许您锁定输入模式、进入网页配置器的能力或两者。启用网页配置器锁定后，如果您希望进行进一步更改，则需要重新刷写固件。
- `4 向摇杆模式` - 启用 4 向摇杆模式，将防止非主方向输入被注册。当接收到第二个主方向输入时，游戏控制器方向将切换到最新的主方向，类似于 [SOCD 后输入优先](../../hotkeys.mdx#socd-后输入优先)。
- `档案` - 启用不同的 GPIO 引脚映射档案。可通过网页配置器或 [快捷键](../../hotkeys.mdx#加载档案-1-4) 进行更改。
- `去抖动延迟（以毫秒为单位）` - 固件忽略额外输入以防止开关干扰意外触发非预期输入的时间帧。
- `使用游戏控制器输入控制显示迷你菜单` - 允许方向输入和按钮控制 `迷你菜单`。

:::note

请注意，如果启用了 `使用游戏控制器输入控制显示迷你菜单`，在导航 `迷你菜单` 时输入仍会发送到设备。
...
![GP2040-CE 配置器 - 输入模式引导](../../assets/images/gpc-settings-boot-input-modes.png)

对于菜单中列出的每个按钮，您可以分配一个不同的输入模式，在启动时按住该按钮进入（例如插入游戏控制器时或重置游戏控制器后立即）。

可选择的输入模式列表，请参阅 [输入模式](../../usage.mdx#输入模式)。

## 快捷键设置

![GP2040-CE 配置器 - 输入模式引导](../../assets/images/gpc-settings-hotkey.png)
...
当 `Function` 键未映射时，您将无法启用 `Fn` 开关。`Fn` 开关开启时需要按下 `Function` 键以及其他指定按键才能触发指定 `快捷键`。

:::note

请注意，灰色的 `Fn` 开关并不意味着 `快捷键` 未激活。灰色的 `Fn` 开关表示未分配 `Function` 键。

如果未分配 `Function` 键，您将在页面顶部看到警告。
